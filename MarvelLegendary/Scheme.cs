...
namespace MarvelLegendary
{
    ...
    public static class SchemeRepository
    {
        private static readonly List<SchemeInfo> _schemes = new List<SchemeInfo>()
        {
            ...
            new SchemeInfoBuilder().SetSchemeName("Asgard Under Siege").SetSchemeSet(Set.P1).AddAdditionalHenchmen(1).CannotBeSolo().SchemeId(100).Duplicates(new List<int>{3, 100, 191}).Build(),
            new SchemeInfoBuilder().SetSchemeName("Destroy the Cities of Earth!").SetSchemeSet(Set.P1).SetBystanderCount(12).SchemeId(101).Duplicates(new List<int>{2, 101, 188}).Build(),
            new SchemeInfoBuilder().SetSchemeName("Enslave Minds with the Chitauri Scepter").SetSchemeSet(Set.P1).SetHeroCount(6).SetRequiredVillains("Chitauri", Set.P1).SetRandomHeroCardsInVillainDeck(12).SchemeId(102).Duplicates(new List<int>{6, 102, 194}).Build(),
            new SchemeInfoBuilder().SetSchemeName("Invade Asgard").SetSchemeSet(Set.P1).SetSchemeTwists(7).SchemeId(103).Duplicates(new List<int>{4, 103, 192}).Build(),
            new SchemeInfoBuilder().SetSchemeName("Radioactive Palladium Poisoning").SetSchemeSet(Set.P1).SetWoundCount(true, 6).SchemeId(104).Duplicates(new List<int>{1, 104, 190}).Build(),
            new SchemeInfoBuilder().SetSchemeName("Replace Earth's Leaders with HYDRA").SetSchemeSet(Set.P1).SetSchemeTwists(5).SetTwistsNextToScheme(3).SetBystanderCount(18).SchemeId(105).Duplicates(new List<int>{5, 105, 193}).Build(),
            new SchemeInfoBuilder().SetSchemeName("Super Hero Civil War").SetSchemeSet(Set.P1).CannotBeSolo().SetSchemeTwists(new List<int> { 0,8,8,5,5}).SetHeroCount(new List<int> {0,4,5,5,6}).SchemeId(106).Duplicates(new List<int>{7, 106, 195}).Build(),
            new SchemeInfoBuilder().SetSchemeName("Unleash the Power of the Cosmic Cube").SetSchemeSet(Set.P1).SchemeId(107).Duplicates(new List<int>{8, 107, 196}).Build(),
            ...
            new SchemeInfoBuilder().SetSchemeName("Enshrouded Identity").SetSchemeTwists(new List<int> { 4,5,6,7,8}).SetEnshroudedGame().SetSchemeSet(Set.Core2E).SchemeId(189).Build(),
